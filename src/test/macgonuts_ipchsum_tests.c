...
CUTE_TEST_CASE(macgonuts_eval_ipchsum_tests)
    struct test_ctx {
    ...
    } test_vector[] = {
        {
            (const unsigned char *)"\x45\x00\x00\x3C\x1C\x46\x40\x00\x40\x06"
                                   "\x00\x00\xAC\x10\x0A\x63\xAC\x10\x0A\x0C", 20, 0xB1E6
        },
        {
            (const unsigned char *)"\x20\x01\xCA\xFE\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02"
                                   ...
                                   "\x00\x00\x00\x20\x00\x00\x00\x00\x00\x3A" // INFO(Rafael): IP6 pseudo-header.
                                   "\x87\x00\x00\x00\x00\x00"
    ...
    CUTE_ASSERT(macgonuts_eval_ipchsum(NULL, 20) == 0);
    CUTE_ASSERT(macgonuts_eval_ipchsum(test, 0) == 0);
    while (test != test_end) {
        uint16_t sum = macgonuts_eval_ipchsum(test->data, test->data_size);
        CUTE_ASSERT(macgonuts_eval_ipchsum(test->data, test->data_size) == test->expected);
        test++;
    }
CUTE_TEST_CASE_END
