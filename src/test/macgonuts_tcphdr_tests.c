...
CUTE_TEST_CASE(macgonuts_make_tcp_pkt_tests)
    unsigned char dgram_from_wire[] = { 0x00, 0x50, 0xB2, 0x2A,
                                        0x00, 0x17, 0x75, 0xC6,
                                        0x1C, 0x00, 0xD4, 0x42,
                                        0x50, 0x18, 0xFF, 0xFF,
                                        0xC7, 0xB3, 0x00, 0x00,
    ...
    free(pkt);
    memcpy(&pheader.src_addr[0], (uint8_t *)"\x0A\x00\x02\x0F", 4);
    memcpy(&pheader.dest_addr[0], (uint8_t *)"\xA8\xEB\x45\xE0", 4);
    memcpy(&pheader.zprotolen[0], (uint8_t *)"\x00\x06\x00\x62", 4);
